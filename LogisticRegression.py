...
for label, value in NumericOneZero.items():
    if not value:
        print(f"Index : {label}, Value : {value}")
        LoanPrep[label] = scalar_.fit_transform(LoanPrep[[label]])


# LoanPrep['income'] = scalar_.fit_transform(LoanPrep[['income']])
# x LoanPrep['loanamt'] = scalar_.fit_transform(LoanPrep[['loanamt']])
...
X_train, X_test, Y_train, Y_test = train_test_split(X_, Y_, test_size=0.3, random_state=1234, stratify=Y_)
